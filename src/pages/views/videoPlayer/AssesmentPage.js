...
function AssesmentPage({ dataPageValue, title,user }) {
  const navigate = useNavigate();
  const language = localStorage.getItem("lang") || "english";

  // console.log(title, "data");

  const AssessmentPageNav = () => {
    navigate("/assessent_page", {
      state: { data: dataPageValue, title: title,id:user },
    });
  };
  return (
    <div className="practise-full-container">
      ...
      <button className="startpractise-btn" onClick={AssessmentPageNav}>
        {language == "english" ? "Start Test" : "தேர்வைத தொடங்கு"}
      </button>
    </div>
  );
}
